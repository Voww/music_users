...
<%Long id = 5L; %>
<%try { %><%id = Long.parseLong(request.getParameter("address_id"));%> <%} catch (NumberFormatException e) { %><%} %>
...
<form action="AdminInstancesServlet" method="post" name="action">
  ...
  <%} else { %>
	...
	<td>
		<form action="AdminInstancesServlet" method="get" name="action_<%=address.getId() %>">
            <input type="hidden" title="table" name="table" value="address">
            <input type="hidden" title="id" name="address_id" value="<%=address.getId() %>">
